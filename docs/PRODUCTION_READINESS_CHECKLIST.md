...
- 🟡 **Provider runs read from Convex** — query APIs, a terminal-run read adapter with local fallback, selected-RFQ optional browser Convex hydration, operator-visible Provider review plus Integration health read-source/fallback health, shared provider-domain copy/state, and deterministic read-history summaries exist; this branch adds local persistence snapshots for future persisted read-health surfaces. → **Slice H (optional)**
