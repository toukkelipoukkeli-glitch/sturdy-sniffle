...
- 🟡 **Convex actually called by the app** — optional browser bridge capability health, configured/missing capability details, bridge recovery actions, and copyable bridge diagnostics are now visible in Integration health, and local fallback remains the default when the bridge is missing or partial. A first-class `ConvexReactClient`/`ConvexProvider` wiring is still future work. → **Slice H (optional, guarded so local-first e2e stays green)**
