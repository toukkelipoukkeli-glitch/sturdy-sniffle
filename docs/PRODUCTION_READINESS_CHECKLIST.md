...
Last updated: 2026-08-02 (Europe/Helsinki). Owner: autonomous build loop.
...
- ✅ Operator can select RFQs, see workload/capacity/material/outside-services/approval/release panels plus a selected-RFQ workspace audit feed with copyable review export and reload coverage for persisted workspace actions (`App.tsx` triage/costing/offer views).
...
- 🟡 **Persistence** — localStorage now restores operator-owned workspace state (RFQs, selected view, edits, actions/status, offer lifecycle events) across reloads; manual RFQs now generate stable customer-ready offer numbers for local export artifacts; Convex-backed app reads remain optional/future. → **Slice H (optional Convex)**
- 🟡 **Loading/empty/stale/error polish** — queue empty states, reload-safe demo import review states, selected-RFQ audit visibility, dense-control accessibility smoke, provider-run read loading/fallback diagnostics, calendar outcome read loading/fallback diagnostics, offer email draft package read-source visibility, offer release execution read-source loading/fallback visibility plus Integration health recovery and diagnostic export surfacing, follow-up activity read-source loading/fallback visibility plus Integration health recovery and diagnostic export surfacing, provider readiness read-source loading/fallback visibility plus diagnostic export surfacing, and a tested workspace error boundary with desktop/mobile recovery coverage exist; broader skeleton/loading and stale-data states remain pending. → **Slice D**
...
- 🟡 **Sheet metal / plastics / wire-EDM / fabrication reachable from UI** — workspace CNC pricing now routes through the shared `calculateQuote` registry, and the other 4 engines surface guarded registry demos with a process selector, best-price/fastest-lead/draft-coverage badges, selected-vs-best comparison summary, quote breakdowns, top assumptions, review flags, an operator checklist, input-readiness groups, planned field descriptors, fixture draft values, explicit quote-path promotion blockers, domain adapter readiness, preview-only edit controls, offer-candidate handoff summary, deterministic promotion-plan contract/panel, local promotion-plan persistence records/snapshot, deterministic promotion action summaries, ready-only promotion draft payloads, canonical command-package descriptors in the review surface/domain layer and preview UI, dry-run/commit execution audit records surfaced in the UI, local execution persistence snapshots, execution history UI, deterministic outcome drafts, outcome draft UI, reviewed-outcome commit adapter plans, commit-plan UI, local outcome commit persistence, outcome commit history UI, operator-visible promoted quote read models, operator-visible deterministic promoted quote application plans, operator-visible local application-plan persistence snapshots, operator-visible deterministic application execution audit records, local application execution persistence snapshots, application execution history UI, application execution outcome drafts, application execution outcome draft UI, application outcome commit plans, application outcome commit-plan UI, local application outcome commit persistence, application outcome commit history UI, operator-visible deterministic application outcome commit read models, deterministic application mutation package descriptors, operator-visible mutation package review surfaces, deterministic mutation package execution audits, local mutation execution persistence snapshots, operator-visible mutation execution history, deterministic mutation execution outcome drafts, operator-visible mutation execution outcome draft review UI, deterministic mutation outcome commit plans/runs, operator-visible mutation outcome commit-plan UI, local mutation outcome commit persistence snapshots, operator-visible mutation outcome commit history, deterministic mutation outcome commit read models, operator-visible mutation outcome commit read-model surfaces, deterministic mutation apply-plan descriptors, operator-visible mutation apply-plan surfaces, local mutation apply-plan persistence snapshots, operator-visible mutation apply-plan history, deterministic mutation apply execution audit records, operator-visible mutation apply execution audits, local mutation apply execution persistence snapshots, operator-visible mutation apply execution history plus desktop/mobile apply-history smoke coverage, an operator-visible deterministic release-readiness card for persisted successful mutation apply executions, an operator-visible deterministic offer-wiring readiness boundary, a domain-only customer-offer creation plan boundary, deterministic customer-offer creation execution audit records, local customer-offer creation execution persistence snapshots, deterministic customer-offer creation execution history summaries, operator-visible local customer-offer creation outcome commit history, deterministic customer-offer export package execution audits, local customer-offer export package execution persistence snapshots, deterministic customer-offer export package execution history summaries, and operator-visible local customer-offer export package execution history for future customer-offer adapters. Live non-CNC offer draft/export creation and release adapter integration remain pending. → **Slice E**
- 🟡 **Non-CNC customer-offer creation history UI** — locally persisted customer-offer creation execution summaries now surface in the guarded registry preview as deterministic review data; active RFQ quote, offer, release, and connector state remain unchanged until a future adapter explicitly applies the package. → **Slice E**
- 🟡 **Non-CNC customer-offer creation outcome commits** — prepared dry-run customer-offer creation executions now produce deterministic review-only outcome suggestions, the guarded registry preview surfaces blocked/ready outcome-draft status, a domain-only reviewed-outcome commit boundary converts ready suggestions into a deterministic commit audit run, and local persistence/history/read-model snapshots plus an operator-visible read-model surface expose reviewed commit runs; ready read models now feed deterministic future-adapter export package descriptors that are surfaced in the guarded preview and can produce deterministic export package execution audit records while active RFQ quote, offer, release, export, and connector state remain unchanged. → **Slice E**
- 🟡 **Non-CNC customer-offer export package execution persistence** — deterministic export package execution audits now record clone-safe local persistence snapshots with artifact outcome counts, source/release evidence IDs, blocked-run withholding, seeded-record validation, and deduped execution fingerprints while live customer-offer, file, release-review, and connector writes remain deferred. → **Slice E**
- 🟡 **Non-CNC customer-offer export package execution history UI** — local export package execution snapshots now produce deterministic history summaries and surface them in the guarded registry preview with artifact outcome totals, blocked/ready status, newest-run evidence, action items, source/release ID withholding, selectable export text, and desktop/mobile smoke coverage while live customer-offer, file, release-review, and connector writes remain deferred. → **Slice E**
- 🟡 **Non-CNC customer-offer export package local provider** — ready export package descriptors can now produce deterministic local/mock artifact outcomes, an operator-visible provider-result read model, local persistence/history snapshots plus a guarded history UI, an operator-visible deterministic commit-boundary status surface, local provider-commit persistence/history summaries surfaced in the guarded registry preview, and an operator-visible final readiness gate that only exposes future live-adapter readiness after matching ready provider-commit evidence while blocked/malformed plans withhold outcomes and live customer-offer, file, release-review, and connector writes remain deferred. → **Slice E**
...
- ✅ **Connector drill-down (linked RFQ/calendar details)** — integration health exposes linked Gmail/calendar records, provider filters, attention empty states, cross-RFQ history, reload-safe stale/blocked link recovery actions, and connector activity rows for the selected RFQ.
- 🟡 **Calendar follow-up + RFQ due-date planning surfaced** — offer release follow-up drafts and selected-RFQ quote work/due drafts are visible before connector execution; deterministic mock connector scheduling now shows linked calendar events; overdue/cancelled follow-up status rows now include deterministic reschedule-ready/blocked previews plus provider-safe reschedule command planning, local persistence snapshots, operator read-model copy/status, deterministic execution audit rows, clone-safe in-memory execution persistence snapshots, execution read-model copy/status, operator-visible dry-run execution status, operator-visible execution history summaries/export copy with desktop/mobile reload coverage, local/mock provider outcome adapter/read-model status/actions, local provider outcome persistence snapshots, provider outcome history summaries/export copy, an operator-visible provider outcome history card/export copy with desktop/mobile reload coverage, local/Convex-style provider outcome read adapters, optional browser-bridge hydration, operator-visible read-source status copy, Integration health visibility for reviewed reschedule commands, deterministic calendar outcome read recovery actions, and copyable calendar outcome read diagnostics; persisted follow-up activity reads now have duplicate-safe write planning plus operator-visible pending/local/Convex/fallback read-source state and pending/partial/recorded/review readiness metadata surfaced in the workspace, with deterministic readiness history summarization/local persistence, deterministic readiness read-model boundary, operator-visible readiness history/read-model status/actions/diagnostic export copy, Integration health persisted-read status/action surfacing and diagnostic copy with explicit readiness-history context, operator-visible history UI, reload-safe follow-up-readiness local-state persistence, unique manual follow-up audit keys, restored-action readiness replay, Convex-safe readiness payload contracts, workflow persistence, client adapter, runtime bridge, operator-visible local/Convex sync-source summaries, scoped sync-health fallback visibility, read/write fallback history details, reload-safe sync-health replay, global fallback indicator alignment, restored sync-health retention capping, per-operation latest fallback visibility, per-operation sync-health status labels, sync-health recovery actions, fallback recency state, sync-health operator summaries, sync-health severity labels, Integration health surfacing for warning/critical fallback state, always-visible topbar persistence alerting for stale fallback state, topbar alert drill-down to detailed sync panels, filterable recent fallback events, zero-count fallback filter empty states, hidden retained fallback overflow copy, and copyable sync-health diagnostic summaries. Real connector scheduling remains behind provider boundaries. → **Slice G**
...
- ✅ **Provider run history filterable in UI** — `ProviderRunReviewPanel` exposes provider run summary metrics plus all, failed, fallbacks, skipped, succeeded, warnings filters, and deterministic provider-read fallback diagnostics backed by `providerRunHistory.ts`.
- 🟡 **Provider runs read from Convex** — query APIs, a terminal-run read adapter with local fallback, selected-RFQ optional browser Convex hydration, operator-visible Provider review plus Integration health read-source/fallback health, shared provider-domain copy/state, deterministic read-history summaries, local persistence snapshots, operator diagnostics/export copy, Provider review UI surfaces for reading/copying those diagnostics, Integration health diagnostic status/recovery/export copy, deterministic next-action items, compact recent-read record drill-downs, provider-readiness persisted-read source visibility plus Integration health actions, optional bridge capability health, configured/missing capability details, bridge recovery actions, copyable bridge capability diagnostics with recovery actions, a tested bridge-health domain helper, a tested browser bridge probe adapter, deterministic browser bridge identity-map readiness for RFQ/offer/quote local ID maps, shared guarded ID-map normalization, deterministic public Convex browser runtime URL probing, Integration health runtime readiness copy, a guarded browser bridge install-plan boundary, Integration health install-plan readiness actions, an explicit opt-in installer decision boundary, and Integration health installer opt-in status copy exist. → **Slice H (optional)**
...
- 🟡 **Offer/release pipeline actionable** — copy/download shipped (Slice A); lifecycle controls shipped (Slice A2); local release execution controls now record deterministic command outcomes, provider-readiness reload/loading/fallback coverage, email draft package history/read-source coverage plus Integration health local read-source surfacing, provider outcome history/read-source coverage plus Integration health local read-source surfacing, and visible pending/local/Convex/fallback read-source state plus Integration health recovery surfacing for provider readiness, release execution history, and follow-up activity history; real connector-backed send remains pending. → **Slice D/G**
...
- 🟡 **e2e breadth** — manual creation, desktop/mobile validation recovery, guarded non-CNC selector previews, estimator-summary copy, mutation apply-history coverage, offer-wiring readiness coverage, offer-creation history coverage, export-package execution-history coverage, queue no-results recovery, workspace error-boundary recovery, reload-safe demo import pre-write review, reload-safe Gmail RFQ sync import recovery, selected-RFQ audit feed visibility/copy/reload recovery, provider-read loading/fallback diagnostics, provider-readiness read loading/fallback visibility, offer release execution read loading/fallback visibility with Integration health recovery and diagnostic copy assertions, follow-up activity read loading/fallback visibility with Integration health recovery and diagnostic copy assertions, local email draft package and provider outcome Integration health diagnostic copy assertions, provider-run filter empty states, follow-up readiness fallback diagnostics, offer reply state filter/empty-state coverage, release-gate review reload coverage, offer lifecycle reload/terminal-state coverage, local provider-readiness release execution reload coverage, email draft package history reload coverage, provider outcome history/read-source reload coverage, calendar reschedule execution-history reload/export coverage, calendar outcome history reload coverage, calendar outcome read fallback diagnostics, restored stale/blocked connector recovery actions, CAD review override recovery, CAD drawing/metadata fallback preview coverage, and desktop/mobile offer copy/text/PDF download coverage are now expanding through deterministic Playwright smoke paths; broader loading/stale-state e2e coverage remains pending. → grows with each slice
