...
- 🟡 **Thumbnails / real previews** — attachment-specific preview and thumbnail labels now distinguish CAD models, drawings, photos, spreadsheets, and metadata-only cards; safe browser-native image attachments render as real `<img>` previews, safe PDF drawing attachments render as browser document previews, and successful STEP/DXF metadata adapter results now mark preview descriptors ready and render compact viewport metadata cards with boundary-aware filename matching while parser failures still use deterministic placeholders. → **Slice F**
