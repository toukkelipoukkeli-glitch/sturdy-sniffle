...
- 🟡 **Sheet metal / plastics / wire-EDM / fabrication reachable from UI** — workspace CNC pricing now routes through the shared `calculateQuote` registry, and the other 4 engines surface guarded read-only registry demos with a process selector, quote breakdowns, top assumptions, review flags, an operator checklist, and a copyable review summary. No non-CNC editable inputs or offer path yet. → **Slice E**
