...
- 🟡 **Thumbnails / real previews** — attachment-specific preview and thumbnail labels now distinguish CAD models, drawings, photos, spreadsheets, and metadata-only cards; safe browser-native image attachments render as real `<img>` previews and safe PDF drawing attachments render as browser document previews, while STEP/DXF still use deterministic placeholders. → **Slice F**
