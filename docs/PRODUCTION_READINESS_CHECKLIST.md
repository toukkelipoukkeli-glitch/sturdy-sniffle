...
- 🟡 **e2e breadth** — manual creation, desktop/mobile validation recovery, guarded non-CNC selector previews, queue no-results recovery, demo import pre-write review, selected-RFQ audit feed visibility/copy, and desktop/mobile offer-copy coverage are now expanding through deterministic Playwright smoke paths; broader error-state e2e coverage remains pending. → grows with each slice
