...
- 🟡 **Sheet metal / plastics / wire-EDM / fabrication reachable from UI** — workspace CNC pricing now routes through the shared `calculateQuote` registry, and the other 4 engines surface guarded read-only registry demos with quote breakdowns. No non-CNC process selector, editable inputs, or offer path yet. → **Slice E**
