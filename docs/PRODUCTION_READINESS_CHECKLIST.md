...
- 🟡 **Convex actually called by the app** — optional browser bridge capability health, configured/missing capability details, bridge recovery actions, copyable bridge diagnostics with recovery actions, a tested deterministic bridge-health helper, and a tested browser bridge probe adapter are now visible in Integration health, and local fallback remains the default when the bridge is missing or partial. A first-class `ConvexReactClient`/`ConvexProvider` wiring is still future work. → **Slice H (optional, guarded so local-first e2e stays green)**
