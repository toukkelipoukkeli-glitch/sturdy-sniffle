...
- 🟡 **Sheet metal / plastics / wire-EDM / fabrication reachable from UI** — workspace CNC pricing now routes through the shared `calculateQuote` registry, and the other 4 engines surface guarded read-only registry demos with a process selector, best-price/fastest-lead/draft-coverage badges, selected-vs-best comparison summary, quote breakdowns, top assumptions, review flags, an operator checklist, blocked editable-input readiness groups, planned field descriptors, read-only fixture draft values, explicit quote-path promotion blockers, and domain adapter readiness in the visible and copyable review surface. Sheet-metal, plastics, wire/EDM, and fabrication planned-field edit plumbing exists behind a shared domain registry path; non-CNC editable UI controls and offer path remain pending. → **Slice E**
