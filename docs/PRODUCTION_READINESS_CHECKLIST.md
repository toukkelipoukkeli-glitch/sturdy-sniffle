...
- 🟡 **Thumbnails / real previews** — attachment-specific preview and thumbnail labels now distinguish CAD models, drawings, photos, spreadsheets, and metadata-only cards; safe browser-native image attachments render as real `<img>` previews, while STEP/DXF/PDF still use deterministic placeholders. → **Slice F**
