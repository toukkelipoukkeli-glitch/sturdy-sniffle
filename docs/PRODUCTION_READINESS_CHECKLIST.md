...
- 🟡 **e2e breadth** — manual creation, desktop/mobile validation recovery, filtered queue empty-state recovery, guarded non-CNC selector previews, and desktop/mobile offer-copy coverage are now expanding through deterministic Playwright smoke paths; broader error-state e2e coverage remains pending. → grows with each slice
