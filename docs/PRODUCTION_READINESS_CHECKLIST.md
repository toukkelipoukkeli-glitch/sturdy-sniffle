...
- 🟡 **Loading/empty/stale/error polish** — queue empty states, reload-safe demo import review states, selected-RFQ audit visibility, and a tested workspace error boundary with desktop/mobile recovery coverage exist; broader skeleton/loading and stale-data states remain pending. → **Slice D**
...
- 🟡 **Offer/release pipeline actionable** — copy/download shipped (Slice A); lifecycle controls shipped (Slice A2); local release execution controls now record deterministic command outcomes, provider-readiness reload coverage, email draft package history coverage, and provider outcome history coverage; real connector-backed send remains pending. → **Slice D/G**
