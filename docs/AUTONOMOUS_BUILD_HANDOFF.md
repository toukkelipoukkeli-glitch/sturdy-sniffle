...
- Main branch checkpoint: `bb3b195` (`Add non-CNC promoted quote application plan (#189)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E non-CNC promoted quote application-plan UI PR.
- In-flight PR at this checkpoint: `codex/non-cnc-application-plan-ui` surfaces the deterministic application plan for promoted non-CNC quote read models while keeping RFQ quote, offer, and release state unchanged.
