...
- Main branch checkpoint: `f8fc3bf` (`Add live adapter execution outcome drafts (#575)`).
...
- The build loop has been using small `codex/*` branches, CodeRabbit review rounds, GitHub CI, and local Bun validation before merge. GitHub Actions did not register PR or post-merge `main` runs for `#575` on 2026-08-06 even though the workflow still declares `pull_request` and `push` to `main`; continue to watch the next hosted run and rely on local full-gate evidence plus review-bot state when Actions does not surface a run.
...
- GitHub Actions: PR/post-merge runs did not register for `#575` on 2026-08-06. Treat this as a hosted trigger gap unless it repeats on the next PR; keep running the full local gate, watch `main`, and record any recurrence.
