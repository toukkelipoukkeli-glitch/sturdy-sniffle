...
- Main branch checkpoint: `5a4e903` (`Dedupe follow-up activity release writes (#268)`).
- Open PRs at this checkpoint: none on `main`; this branch is extending the duplicate-suppression path so manual/operator follow-up creation also avoids repeated remote Convex writes while preserving local workspace action history.
- In-flight PR at this checkpoint: `codex/followup-activity-manual-dedupe` adds a Convex workspace persistence write predicate, passes it through the runtime bridge, and uses persisted follow-up activity task IDs to skip duplicate remote follow-up activity writes without dropping local fallback state.
