...
- Main branch checkpoint: `d6f8b54` (`Surface follow-up readiness sync source (#282)`).
- Open PRs at this checkpoint: none on `main`; this branch is surfacing follow-up activity readiness sync health in the Offer workspace.
- In-flight PR at this checkpoint: `codex/followup-readiness-sync-health` makes workspace persistence fallback status visible next to follow-up readiness history while keeping local fallback behavior deterministic.
