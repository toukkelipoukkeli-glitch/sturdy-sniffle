...
- Main branch checkpoint: `8d5bb5c` (`Surface calendar provider outcome read model (#362)`).
...
- Calendar planning for RFQ due holds and offer follow-ups behind adapter boundaries, including local/mock provider outcomes, read-model copy/status, and operator-visible provider outcome cards for reviewed reschedule commands while live Calendar writes remain deferred.
