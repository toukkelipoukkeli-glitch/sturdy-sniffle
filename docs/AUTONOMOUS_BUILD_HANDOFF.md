...
- Main branch checkpoint: `0c97bf1` (`Refresh offer creation outcome commit persistence handoff (#520)`).
- Open PRs at this checkpoint: none on `main`.
- In-flight branch at this checkpoint: `codex/non-cnc-offer-creation-outcome-commit-history-ui` is surfacing locally persisted customer-offer creation outcome commit history in the guarded non-CNC registry preview without mutating active RFQ quote, offer, release, export, or connector state.
