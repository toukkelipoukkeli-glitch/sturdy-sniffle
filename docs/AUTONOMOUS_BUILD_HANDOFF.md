...
- Main branch checkpoint: `76e18cc` (`Refresh calendar provider outcome persistence handoff (#365)`).
- Open PRs at this checkpoint: none.
- In-flight branch at this checkpoint: `codex/calendar-provider-outcome-history-summary` adds deterministic history summaries/export copy for persisted calendar provider outcome batches.
...
- Calendar planning for RFQ due holds and offer follow-ups behind adapter boundaries, including local/mock provider outcomes, read-model copy/status, operator-visible provider outcome cards, local provider outcome persistence snapshots, and in-flight provider outcome history summaries/export copy for reviewed reschedule commands while live Calendar writes remain deferred.
