...
- Main branch checkpoint: `f744ecf` (`Add application outcome commit read model (#203)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI surface for the non-CNC promoted quote application outcome commit read model.
- In-flight PR at this checkpoint: `codex/application-outcome-read-model-ui` surfaces the deterministic application outcome commit read model in the non-CNC registry preview while keeping active RFQ quote, offer, and release state unchanged.
