...
- Main branch checkpoint: `be9ffc5` (`Add mutation apply execution audits (#222)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI surface for non-CNC promoted quote application mutation apply execution audits.
- In-flight PR at this checkpoint: `codex/application-mutation-apply-execution-ui` surfaces deterministic mutation apply execution audit records in the non-CNC registry preview while keeping active RFQ quote, offer, and release state unchanged.
