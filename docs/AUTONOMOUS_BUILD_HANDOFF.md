...
- Main branch checkpoint: `9122bfa` (`Add application mutation execution persistence (#208)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI history surface for non-CNC promoted quote application mutation execution audits.
- In-flight PR at this checkpoint: `codex/application-mutation-execution-history-ui` surfaces deterministic application mutation execution audit/history panels in the non-CNC registry preview while keeping active RFQ quote, offer, and release state unchanged.
