...
- Main branch checkpoint: `daef72d` (`Seed fallback indicator from sync health replay (#286)`).
- Open PRs at this checkpoint: none on `main`; this branch is capping restored follow-up readiness sync-health fallback histories to the same bounded window used for live recording.
- In-flight PR at this checkpoint: `codex/followup-readiness-sync-health-cap` applies the existing 12-event sync-health retention limit to restored local workspace state so stale fallback histories cannot grow unbounded after reload.
