...
- Main branch checkpoint: `ac1c576` (`Add application mutation outcome drafts (#210)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI surface for non-CNC promoted quote application mutation execution outcome drafts.
- In-flight PR at this checkpoint: `codex/application-mutation-outcome-draft-ui` surfaces deterministic mutation execution outcome drafts in the non-CNC registry preview UI while keeping active RFQ quote, offer, and release state unchanged.
