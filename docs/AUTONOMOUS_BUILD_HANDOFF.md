...
- Main branch checkpoint: `327d95a` (`Add follow-up readiness Convex payloads (#278)`).
- Open PRs at this checkpoint: none on `main`; this branch is adding Convex workflow persistence for follow-up activity readiness snapshots.
- In-flight PR at this checkpoint: `codex/followup-readiness-workflow-api` adds a tenant-safe `offerFollowUpActivityReadiness` table plus list/record workflow APIs that upsert by readiness key, validate counts, and append an audit activity.
