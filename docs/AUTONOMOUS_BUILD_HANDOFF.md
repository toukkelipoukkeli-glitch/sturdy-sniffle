...
- Main branch checkpoint: `8a7e23e` (`[codex] Add follow-up readiness sync health history (#284)`).
- Open PRs at this checkpoint: none on `main`; this branch is adding reload-safe local replay for follow-up readiness sync-health fallback events.
- In-flight PR at this checkpoint: `codex/followup-readiness-replay-health` persists validated follow-up readiness sync-health fallback events in the local workspace state and restores them after reload without changing Convex/local persistence behavior.
