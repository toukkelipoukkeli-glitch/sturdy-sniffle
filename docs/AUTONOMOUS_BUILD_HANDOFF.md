...
- Main branch checkpoint: `00dcfb2` (`Add follow-up sync health export summary (#300)`).
- Open PRs at this checkpoint: none on `main`; this branch is adding a deterministic Convex provider-run read adapter for terminal provider audit records.
- In-flight PR at this checkpoint: `codex/provider-run-convex-read-adapter` hydrates completed Convex provider-run records into local audit history with query fallback behavior.
...
- Provider adapter boundaries for mock/local/provider AI work, with Convex-backed provider run audit records, query APIs, and an in-flight terminal-run read adapter boundary for future UI hydration.
