...
- Main branch checkpoint: `05857b6` (`Surface mutation apply plan history (#221)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E domain execution audit for non-CNC promoted quote application mutation apply plans.
- In-flight PR at this checkpoint: `codex/application-mutation-apply-execution` builds deterministic dry-run/commit mutation apply execution audit records while keeping active RFQ quote, offer, and release state unchanged.
