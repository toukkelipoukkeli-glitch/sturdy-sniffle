...
- Main branch checkpoint: `810f6a9` (`Surface application execution history (#196)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E domain boundary for non-CNC promoted quote application execution outcome drafts.
- In-flight PR at this checkpoint: `codex/non-cnc-application-outcome-drafts` adds deterministic suggested outcome drafts for ready promoted quote application records while keeping active RFQ quote, offer, and release state unchanged.
