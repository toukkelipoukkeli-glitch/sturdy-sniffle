...
- Main branch checkpoint: `2ba2f20` (`Add follow-up sync health severity (#293)`).
- Open PRs at this checkpoint: none on `main`; this branch is lifting follow-up readiness sync-health alerts into the top-level Integration health persistence row.
- In-flight PR at this checkpoint: `codex/followup-sync-health-integration-alert` surfaces warning/critical follow-up readiness persistence health in the Integration health panel so stale fallback history is visible without drilling into the Offer history card.
