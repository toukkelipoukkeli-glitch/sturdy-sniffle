...
- Main branch checkpoint: `e96bd8b` (`Add offer email draft package history (#247)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing a narrow UI surface for offer email draft package history.
- In-flight PR at this checkpoint: `codex/offer-email-draft-history-ui` surfaces the deterministic email draft package history summary in the offer workspace while actual Gmail/provider sending stays guarded for later.
...
- Offer builder, offer document content, export fixtures, offer lifecycle, release plan, guarded email draft package descriptors, local email draft package persistence snapshots, email draft package history summaries and UI surface, release execution audit/history/fingerprints, and persistence adapters.
...
5. Harden PDF/export verification for offers.
   ...
   - Current branch surfaces the deterministic draft package history summary in the offer workspace.
