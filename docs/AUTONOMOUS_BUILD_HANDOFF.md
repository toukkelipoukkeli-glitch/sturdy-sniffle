...
- Main branch checkpoint: `26374c8` (`Surface mutation outcome commit read model (#217)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E deterministic apply-plan boundary for non-CNC promoted quote application mutation outcome commits.
- In-flight PR at this checkpoint: `codex/application-mutation-apply-plan` derives reviewable apply-plan descriptors from ready/blocked mutation outcome commit read models while keeping active RFQ quote, offer, and release state unchanged.
