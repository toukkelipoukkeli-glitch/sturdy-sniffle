...
- Main branch checkpoint: `9534d03` (`Add non-CNC application persistence (#191)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI surface for non-CNC promoted quote application persistence.
- In-flight PR at this checkpoint: `codex/non-cnc-application-ui-persistence` surfaces local promoted quote application history in the operator workspace while keeping RFQ quote, offer, and release state unchanged.
