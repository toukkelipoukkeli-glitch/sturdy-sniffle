...
- Main branch checkpoint: `f9e7e8e` (`Refresh non-CNC offer wiring UI handoff (#498)`).
- Open PRs at this checkpoint: none on `main`.
- In-flight branch at this checkpoint: `codex/non-cnc-offer-wiring-e2e` — adds deterministic desktop/mobile Playwright coverage for the operator-visible non-CNC offer-wiring readiness boundary.
