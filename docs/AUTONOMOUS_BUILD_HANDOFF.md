...
- Main branch checkpoint: `0b766bd` (`Dedupe manual follow-up activity writes (#269)`).
- Open PRs at this checkpoint: none on `main`; this branch is adding a deterministic read-model/readiness helper for persisted offer follow-up activity summaries.
- In-flight PR at this checkpoint: `codex/followup-activity-readiness` turns follow-up activity read summaries into pending/partial/recorded/review readiness metadata with normalized expected/recorded/missing/unexpected task IDs and operator next actions.
