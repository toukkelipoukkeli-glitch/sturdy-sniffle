...
- Main branch checkpoint: `66fc73d` (`Surface application outcome commit history (#202)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E domain read model for persisted non-CNC promoted quote application outcome commits.
- In-flight PR at this checkpoint: `codex/application-outcome-commit-read-model` adds a deterministic read model that gates future active RFQ quote, offer, and release-state mutation on committed application outcome records.
