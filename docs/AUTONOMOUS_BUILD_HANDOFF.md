...
- Main branch checkpoint: `c81d26b` (`Surface provider read diagnostics (#309)`).
- Open PRs at this checkpoint: none on `main`; this branch makes provider-run read diagnostics copyable from the Provider review panel.
- In-flight PR at this checkpoint: `codex/provider-read-diagnostics-copy` adds a deterministic copy action and status feedback for the selected RFQ provider-run diagnostic export.
