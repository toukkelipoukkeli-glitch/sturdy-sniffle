...
- Main branch checkpoint: `654388e` (`Split follow-up sync health latest fallbacks (#288)`).
- Open PRs at this checkpoint: none on `main`; this branch is classifying follow-up readiness sync-health fallback status by read/write operation mix.
- In-flight PR at this checkpoint: `codex/followup-readiness-sync-health-status` exposes deterministic healthy/read-fallback/write-fallback/read-write-fallback sync-health status labels in the Offer workspace panel.
