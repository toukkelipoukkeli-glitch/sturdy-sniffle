...
- Main branch checkpoint: `59f0a93` (`Surface non-CNC promoted quote read model (#188)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E non-CNC promoted quote application-plan PR.
- In-flight PR at this checkpoint: `codex/non-cnc-promoted-quote-application-plan` adds a deterministic application plan for promoted non-CNC quote read models while keeping RFQ quote, offer, and release state unchanged.
