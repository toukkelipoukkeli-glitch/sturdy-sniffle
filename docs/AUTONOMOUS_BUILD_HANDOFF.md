...
- Main branch checkpoint: `fe160ad` (`Add non-CNC application execution persistence (#195)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI surface for local non-CNC promoted quote application execution history.
- In-flight PR at this checkpoint: `codex/non-cnc-application-execution-history` surfaces local application execution persistence snapshots in the non-CNC registry preview while keeping RFQ quote, offer, and release state unchanged.
