...
- Main branch checkpoint: `b1df68e` (`Add non-CNC promoted quote read model (#187)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E non-CNC promoted quote read-model UI PR.
- In-flight PR at this checkpoint: `codex/non-cnc-promoted-quote-ui` surfaces the deterministic promoted quote read model in the operator workspace while keeping RFQ quote, offer, and release state unchanged.
