...
- Main branch checkpoint: `d0a905b` (`Replay restored follow-up readiness locally (#277)`).
- Open PRs at this checkpoint: none on `main`; this branch is adding a Convex-safe follow-up activity readiness payload contract.
- In-flight PR at this checkpoint: `codex/followup-readiness-convex-payload` maps follow-up activity readiness history records into validated Convex payload/read records so the future workflow mutation can persist readiness snapshots without trusting localStorage shapes.
