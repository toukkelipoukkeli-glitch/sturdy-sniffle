...
- Main branch checkpoint: `5a4bd9a` (`Add non-CNC promotion execution persistence (#179)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E non-CNC promotion execution history UI PR.
- In-flight PR at this checkpoint: `codex/non-cnc-execution-history-ui` surfaces deterministic local execution persistence snapshots in the registry preview while keeping RFQ quote, offer, and release state unchanged.
