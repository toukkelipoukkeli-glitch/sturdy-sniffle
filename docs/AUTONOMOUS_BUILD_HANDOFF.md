...
- Main branch checkpoint: `760f18a` (`Surface provider read diagnostics in integration health (#311)`).
- Open PRs at this checkpoint: none on `main`; this branch makes the Integration health provider-run diagnostics export copyable.
- In-flight PR at this checkpoint: `codex/provider-read-integration-copy` adds deterministic copy/status feedback for selected-RFQ provider-read diagnostics from Integration health.
