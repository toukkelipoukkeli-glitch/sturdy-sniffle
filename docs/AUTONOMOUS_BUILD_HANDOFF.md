...
- Main branch checkpoint: `5301592` (`Surface non-CNC application persistence history (#192)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E non-CNC promoted quote application execution audit boundary.
- In-flight PR at this checkpoint: `codex/non-cnc-application-execution` adds deterministic dry-run/commit execution audit records for promoted quote application records while keeping RFQ quote, offer, and release state unchanged.
