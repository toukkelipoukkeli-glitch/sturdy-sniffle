...
- Main branch checkpoint: `07b7abb` (`Surface mutation outcome commit history (#215)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E read model for non-CNC promoted quote application mutation outcome commits.
- In-flight PR at this checkpoint: `codex/application-mutation-outcome-read-model` derives a deterministic ready/blocked read model from persisted mutation outcome commits while keeping active RFQ quote, offer, and release state unchanged.
