...
- Main branch checkpoint: `8c40766` (`Surface non-CNC outcome commit history (#186)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E non-CNC promoted quote read-model PR.
- In-flight PR at this checkpoint: `codex/non-cnc-promoted-quote-read-model` adds a deterministic read model for successful reviewed non-CNC quote promotion commits while keeping RFQ quote, offer, and release state unchanged.
