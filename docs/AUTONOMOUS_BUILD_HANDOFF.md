...
- Main branch checkpoint: `a137af7` (`Add provider run read history summary (#306)`).
- Open PRs at this checkpoint: none on `main`; this branch adds deterministic local persistence snapshots for provider-run read history records.
- In-flight PR at this checkpoint: `codex/provider-read-history-persistence` records normalized provider-run read history rows with cloned snapshots, record-key dedupe, source/status counters, and seeded-record validation.
