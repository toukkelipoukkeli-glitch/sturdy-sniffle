...
- Main branch checkpoint: `a9a2517` (`Add follow-up readiness workflow APIs (#279)`).
- Open PRs at this checkpoint: none on `main`; this branch is adding the deterministic local/Convex client adapter for follow-up activity readiness persistence.
- In-flight PR at this checkpoint: `codex/followup-readiness-client-adapter` maps validated follow-up readiness history records through the new workflow mutation/query APIs, converts ISO timestamps at the Convex boundary, and keeps local fallback snapshots hot.
