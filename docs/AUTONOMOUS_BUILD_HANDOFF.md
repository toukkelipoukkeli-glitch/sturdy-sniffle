...
- Main branch checkpoint: `e8c5b03` (`Add mutation apply plan persistence (#220)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI surface for non-CNC promoted quote application mutation apply-plan history.
- In-flight PR at this checkpoint: `codex/application-mutation-apply-history-ui` surfaces local mutation apply-plan persistence snapshots in the non-CNC registry preview while keeping active RFQ quote, offer, and release state unchanged.
