...
- Main branch checkpoint: `92299a1` (`Refresh calendar provider outcome UI handoff (#363)`).
- Open PRs at this checkpoint: none.
- In-flight branch at this checkpoint: `codex/calendar-provider-outcome-persistence` adds deterministic local persistence snapshots for calendar reschedule provider outcomes.
...
- Calendar planning for RFQ due holds and offer follow-ups behind adapter boundaries, including local/mock provider outcomes, read-model copy/status, operator-visible provider outcome cards, and in-flight local provider outcome persistence snapshots for reviewed reschedule commands while live Calendar writes remain deferred.
