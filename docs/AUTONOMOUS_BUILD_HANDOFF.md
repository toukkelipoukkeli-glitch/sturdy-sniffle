...
- Main branch checkpoint: `1681dd9` (`Add non-CNC input readiness manifest (#150)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the next Slice E field-plan PR.
- In-flight PR at this checkpoint: `codex/non-cnc-edit-plan` adds deterministic field-level input plans for future editable non-CNC process flows.
...
1. Move the app's quote path toward the multi-process registry.
   ...
   - Read-only non-CNC registry demos now show a guarded process selector, best-price/fastest-lead badges, selected-vs-best summary, quote breakdowns, assumptions, review flags, operator checklist, input-readiness groups, and copyable summary; this branch adds field-level planned inputs before editable non-CNC controls. After this, non-CNC engines need editable process-specific inputs and offer wiring.
