...
- Main branch checkpoint: `ab4ec11` (`Add application mutation package descriptors (#205)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI surface for future non-CNC promoted quote application mutation packages.
- In-flight PR at this checkpoint: `codex/application-mutation-package-ui` surfaces deterministic application mutation package descriptors in the non-CNC registry preview while keeping active RFQ quote, offer, and release state unchanged.
