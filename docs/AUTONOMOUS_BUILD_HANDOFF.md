...
- Main branch checkpoint: `a1dcf2d` (`Surface mutation apply plan in workspace (#219)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E local persistence adapter for non-CNC promoted quote application mutation apply plans.
- In-flight PR at this checkpoint: `codex/application-mutation-apply-persistence` records review-safe local mutation apply-plan summaries with seeded-record validation and deduplication while keeping active RFQ quote, offer, and release state unchanged.
