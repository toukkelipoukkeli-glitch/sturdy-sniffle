...
- Main branch checkpoint: `b04dcca` (`Add provider read history persistence (#307)`).
- Open PRs at this checkpoint: none on `main`; this branch adds deterministic provider-run read history diagnostics/export copy for operator review.
- In-flight PR at this checkpoint: `codex/provider-read-history-diagnostics` summarizes persisted provider-run read history snapshots into status, severity, recovery actions, recent read rows, and copyable diagnostic text.
