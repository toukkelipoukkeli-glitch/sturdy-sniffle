...
- Main branch checkpoint: `e28ecc8` (`Refresh offer creation read model handoff (#525)`).
- Open PRs at this checkpoint: none on `main`.
- In-flight branch at this checkpoint: `codex/non-cnc-offer-export-package-plan` is adding a deterministic non-CNC customer-offer export package plan boundary without creating live customer offers, files, release reviews, or connector side effects.
