...
- Main branch checkpoint: `e6f3e2f` (`Add mutation outcome commit persistence (#214)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI history surface for non-CNC promoted quote application mutation outcome commit audits.
- In-flight PR at this checkpoint: `codex/application-mutation-outcome-commit-history` surfaces deterministic local mutation outcome commit history in the non-CNC registry preview UI while keeping active RFQ quote, offer, and release state unchanged.
