...
- Main branch checkpoint: `ff784f8` (`Add mutation apply plan descriptors (#218)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI surface for non-CNC promoted quote application mutation apply plans.
- In-flight PR at this checkpoint: `codex/application-mutation-apply-plan-ui` surfaces reviewable mutation apply-plan descriptors in the non-CNC registry preview while keeping active RFQ quote, offer, and release state unchanged.
