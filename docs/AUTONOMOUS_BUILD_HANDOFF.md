...
- Main branch checkpoint: `1a3f7c8` (`Classify follow-up sync health status (#289)`).
- Open PRs at this checkpoint: none on `main`; this branch is adding deterministic operator recovery actions for follow-up readiness sync-health fallback states.
- In-flight PR at this checkpoint: `codex/followup-sync-health-recovery-actions` surfaces read/write recovery guidance in the Offer workspace sync-health panel.
