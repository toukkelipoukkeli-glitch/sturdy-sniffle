...
- Main branch checkpoint: `a73efe6` (`Add follow-up sync health recovery actions (#290)`).
- Open PRs at this checkpoint: none on `main`; this branch is adding deterministic current/stale age state for follow-up readiness sync-health fallbacks.
- In-flight PR at this checkpoint: `codex/followup-sync-health-aged-state` surfaces injected-clock fallback recency in the Offer workspace sync-health panel.
