...
- Main branch checkpoint: `d165512` (`Add non-CNC outcome commit persistence (#185)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E non-CNC promotion outcome commit history UI PR.
- In-flight PR at this checkpoint: `codex/non-cnc-outcome-commit-history-ui` surfaces the local reviewed-outcome commit persistence snapshot in the non-CNC preview while keeping RFQ quote, offer, and release state unchanged.
