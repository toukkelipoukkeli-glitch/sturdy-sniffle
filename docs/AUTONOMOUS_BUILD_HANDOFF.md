...
- Main branch checkpoint: `28658d4` (`Add application outcome commit persistence (#201)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI surface for local non-CNC promoted quote application outcome commit history.
- In-flight PR at this checkpoint: `codex/application-outcome-commit-history-ui` surfaces deterministic local application outcome commit persistence records in the non-CNC registry preview while keeping active RFQ quote, offer, and release state unchanged.
