...
- Main branch checkpoint: `cea4860` (`Cap restored follow-up sync health events (#287)`).
- Open PRs at this checkpoint: none on `main`; this branch is splitting latest follow-up readiness sync-health fallback visibility by read and write operation.
- In-flight PR at this checkpoint: `codex/followup-readiness-sync-health-summary` exposes latest read and latest write fallback events in the deterministic sync-health summary and Offer workspace panel.
