...
- Main branch checkpoint: `d1cf322` (`[codex] Extract provider read sync copy (#305)`).
- Open PRs at this checkpoint: none on `main`; this branch adds deterministic provider-run read history summaries for future persisted read-health surfaces.
- In-flight PR at this checkpoint: `codex/provider-read-history-summary` records normalized provider-run read batches with source/status counts, latest/current lookup, and seeded-record validation.
