...
- Main branch checkpoint: `b9a52c2` (`Add application execution outcome drafts (#197)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI surface for non-CNC promoted quote application execution outcome drafts.
- In-flight PR at this checkpoint: `codex/application-outcome-draft-ui` surfaces deterministic application execution outcome drafts in the non-CNC registry preview while keeping active RFQ quote, offer, and release state unchanged.
