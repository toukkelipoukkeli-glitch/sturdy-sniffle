...
- Main branch checkpoint: `fa1f81f` (`Add provider read history diagnostics (#308)`).
- Open PRs at this checkpoint: none on `main`; this branch surfaces provider-run read history diagnostics in the Provider review panel.
- In-flight PR at this checkpoint: `codex/provider-read-diagnostics-ui` shows deterministic read-health status, severity, recovery action, and diagnostic export copy for the selected RFQ provider-run read state.
