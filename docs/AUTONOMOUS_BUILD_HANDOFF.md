...
- Main branch checkpoint: `6fba0a7` (`Surface non-CNC promotion execution audits (#178)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E non-CNC promotion execution persistence PR.
- In-flight PR at this checkpoint: `codex/non-cnc-promotion-execution-persistence` adds deterministic local persistence snapshots for non-CNC promotion execution runs while keeping RFQ quote, offer, and release state unchanged.
