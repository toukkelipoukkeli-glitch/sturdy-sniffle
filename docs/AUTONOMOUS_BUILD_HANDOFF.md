...
- Main branch checkpoint: `7cd757f` (`Surface non-CNC application plan in workspace (#190)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E non-CNC promoted quote application persistence PR.
- In-flight PR at this checkpoint: `codex/non-cnc-application-persistence` adds deterministic local persistence snapshots for promoted quote application plans while keeping RFQ quote, offer, and release state unchanged.
