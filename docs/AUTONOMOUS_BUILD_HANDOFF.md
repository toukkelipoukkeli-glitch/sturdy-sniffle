...
- Main branch checkpoint: `8d522f6` (`Add Convex provider run read adapter (#301)`).
- Open PRs at this checkpoint: none on `main`; this branch is wiring the provider-run read adapter into the optional browser Convex bridge.
- In-flight PR at this checkpoint: `codex/provider-run-runtime-read` hydrates selected-RFQ terminal provider runs into the provider review panel while preserving local fixture fallback.
...
- Provider adapter boundaries for mock/local/provider AI work, with Convex-backed provider run audit records, query APIs, and an in-flight optional browser runtime read path for selected-RFQ terminal provider runs.
