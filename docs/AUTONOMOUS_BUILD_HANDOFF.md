...
- Main branch checkpoint: `d0d43e5` (`Surface application outcome read model (#204)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E domain package boundary for future non-CNC promoted quote application mutations.
- In-flight PR at this checkpoint: `codex/application-mutation-package` adds deterministic application mutation package descriptors from the application outcome commit read model while keeping active RFQ quote, offer, and release state unchanged.
