...
- Main branch checkpoint: `c6ba4a5` (`Add mutation outcome commit read model (#216)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI surface for non-CNC promoted quote application mutation outcome commit read models.
- In-flight PR at this checkpoint: `codex/application-mutation-outcome-read-model-ui` surfaces the deterministic ready/blocked mutation outcome commit read model in the non-CNC registry preview while keeping active RFQ quote, offer, and release state unchanged.
