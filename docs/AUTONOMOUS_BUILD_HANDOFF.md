...
- Main branch checkpoint: `b784751` (`[codex] Surface follow-up readiness sync health (#283)`).
- Open PRs at this checkpoint: none on `main`; this branch is adding follow-up readiness sync-health fallback history details in the Offer workspace.
- In-flight PR at this checkpoint: `codex/followup-readiness-sync-health-history` records deterministic read/write fallback events for follow-up readiness sync health and surfaces the latest fallback operation without changing persistence behavior.
