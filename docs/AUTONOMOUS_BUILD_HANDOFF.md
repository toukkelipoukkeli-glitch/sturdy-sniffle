...
Last refreshed: 2026-07-01 Europe/Helsinki.
...
- Main branch checkpoint: `9b1c809` (`Add mutation outcome commit planning (#212)`).
- Open PRs at this checkpoint: none on `main`; this branch is preparing the follow-up Slice E UI surface for non-CNC promoted quote application mutation outcome commit plans.
- In-flight PR at this checkpoint: `codex/application-mutation-outcome-commit-ui` surfaces deterministic mutation outcome commit plans/runs in the non-CNC registry preview UI while keeping active RFQ quote, offer, and release state unchanged.
