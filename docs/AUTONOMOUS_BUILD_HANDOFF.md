...
- Main branch checkpoint: `29e75bf` (`Add provider read diagnostics copy action (#310)`).
- Open PRs at this checkpoint: none on `main`; this branch surfaces provider-run read diagnostics in the Integration health panel.
- In-flight PR at this checkpoint: `codex/provider-read-diagnostics-integration-health` adds selected-RFQ provider-read diagnostic status, summary, severity, and recovery copy to Integration health.
