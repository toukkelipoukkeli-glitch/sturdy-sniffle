...
- Main branch checkpoint: `7c31b93` (`Persist follow-up readiness history locally (#275)`).
- Open PRs at this checkpoint: none on `main`; this branch is hardening repeated manual follow-up action audit keys.
- In-flight PR at this checkpoint: `codex/followup-action-unique-keys` adds deterministic follow-up action key suffixes for repeated manual follow-up writes so local audit rows remain unique without changing follow-up task IDs or connector dedupe behavior.
