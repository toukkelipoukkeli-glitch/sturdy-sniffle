...
- Main branch checkpoint: `f6c9059` (`Add calendar provider outcome read adapter (#370)`).
- Open PRs at this checkpoint: none.
- In-flight branch at this checkpoint: `codex/calendar-provider-outcome-read-hydration` is wiring the calendar provider outcome history panel to the optional browser Convex read bridge while keeping local fallback authoritative and provider execution deferred.
...
- Calendar planning for RFQ due holds and offer follow-ups behind adapter boundaries, including local/mock provider outcomes, read-model copy/status, operator-visible provider outcome cards, local provider outcome persistence snapshots, provider outcome history summaries/export copy, operator-visible history card/export copy, local/Convex-style provider outcome read adapters, and in-flight browser-bridge hydration for reviewed reschedule command outcomes while live Calendar writes remain deferred.
