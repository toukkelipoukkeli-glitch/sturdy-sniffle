...
test("filters the RFQ queue and discloses attachments", async ({ page }) => {
  ...
  const cadAdapterPreview = partPreview.getByLabel("FB-204-A.step adapter preview")
  await expect(cadAdapterPreview).toContainText("STEP")
  await expect(cadAdapterPreview).toContainText("Aluminum 6082")
  await expect(cadAdapterPreview).toContainText("cnc milling")
  await expect(cadAdapterPreview).toContainText("heuristic")
...
test("runs the quote workspace costing workflow", async ({ page }) => {
  ...
  await expect(page.getByLabel("FB-204-A.step adapter preview")).toContainText("Aluminum 6082")
