...
async function assertOfferExportLiveAdapterFinalGateFollowThroughHistory(nonCncDemos: Locator, page: Page) {
  const liveAdapterFinalGateFollowThroughHistory = nonCncDemos.getByLabel(
    "Non-CNC promoted quote customer export live adapter final gate follow-through history",
    { exact: true },
  )

  await expect(liveAdapterFinalGateFollowThroughHistory).toBeVisible()
  await expect(liveAdapterFinalGateFollowThroughHistory).toHaveAttribute("data-status", "blocked")
  await expect(liveAdapterFinalGateFollowThroughHistory).toContainText(
  ...
  await expect(liveAdapterFinalGateFollowThroughHistory).toContainText("Ready 0, blocked 1")
  await expect(liveAdapterFinalGateFollowThroughHistory).toContainText("0 applied commands")
  await expect(liveAdapterFinalGateFollowThroughHistory).toContainText("Planned 0, blocked 5")
  await expect(liveAdapterFinalGateFollowThroughHistory).toContainText(
    "Resolve final-gate follow-through blockers before enabling release follow-through adapters.",
  )
  await expect(liveAdapterFinalGateFollowThroughHistory).toContainText("Ready follow-throughs: None")
  await expect(liveAdapterFinalGateFollowThroughHistory).toContainText(
    "Blocked follow-throughs: non-cnc-promoted-quote-offer-export-live-adapter-apply-execution-final-gate-follow-through-",
  )
  await expect(liveAdapterFinalGateFollowThroughHistory).toContainText("Readiness records: None")
  await expect(liveAdapterFinalGateFollowThroughHistory).toContainText("Apply plans: None")
  await expect(liveAdapterFinalGateFollowThroughHistory).toContainText("Committed executions: None")
  await expect(liveAdapterFinalGateFollowThroughHistory).toContainText("Source executions: None")
  await expect(liveAdapterFinalGateFollowThroughHistory).toContainText(
    "Non-CNC live adapter final-gate follow-through history",
  )
  await expect(liveAdapterFinalGateFollowThroughHistory).toContainText("Status counts: blocked")
  await assertNoHorizontalOverflow(page)
}

async function assertOfferExportLiveAdapterFinalGateFollowThroughExecutionHistory(nonCncDemos: Locator, page: Page) {
  const liveAdapterFinalGateFollowThroughExecutionHistory = nonCncDemos.getByLabel(
    "Non-CNC promoted quote customer export live adapter final gate follow-through execution history",
    { exact: true },
  )

  await expect(liveAdapterFinalGateFollowThroughExecutionHistory).toBeVisible()
  await expect(liveAdapterFinalGateFollowThroughExecutionHistory).toHaveAttribute("data-status", "blocked")
  await expect(liveAdapterFinalGateFollowThroughExecutionHistory).toContainText(
  ...
  await expect(liveAdapterFinalGateFollowThroughExecutionHistory).toContainText("1 run")
  await expect(liveAdapterFinalGateFollowThroughExecutionHistory).toContainText(
  ...
  await expect(liveAdapterFinalGateFollowThroughExecutionHistory).toContainText("Readiness records: None")
  await expect(liveAdapterFinalGateFollowThroughExecutionHistory).toContainText("Apply plans: None")
  await expect(liveAdapterFinalGateFollowThroughExecutionHistory).toContainText("Commit records: None")
  await expect(liveAdapterFinalGateFollowThroughExecutionHistory).toContainText(
    "Non-CNC live adapter final-gate follow-through execution history",
  )
  await expect(liveAdapterFinalGateFollowThroughExecutionHistory).toContainText("Status counts: blocked")
  await assertNoHorizontalOverflow(page)
}

async function assertOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeDraft(nonCncDemos: Locator, page: Page) {
  const liveAdapterFinalGateFollowThroughExecutionOutcomeDraft = nonCncDemos.getByLabel(
    "Non-CNC promoted quote customer export live adapter final gate follow-through execution outcome draft",
    { exact: true },
  )

  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeDraft).toBeVisible()
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeDraft).toHaveAttribute("data-status", "blocked")
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeDraft).toContainText(
  ...
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeDraft).toContainText("0 ready outcomes")
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeDraft).toContainText("5 blocked outcomes")
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeDraft).toContainText(
...
async function assertOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory(
  nonCncDemos: Locator,
  page: Page,
) {
  const liveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory = nonCncDemos.getByLabel(
    "Non-CNC promoted quote customer export live adapter final gate follow-through execution outcome commit history",
    { exact: true },
  )

  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toBeVisible()
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toHaveAttribute("data-status", "blocked")
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toContainText(
    "Offer export live adapter final gate follow-through outcome commit history",
  )
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toContainText(
    "Final-gate follow-through outcome commit history blocked",
  )
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toContainText(
    "Latest non-CNC live-adapter final-gate follow-through outcome commit is blocked",
  )
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toContainText("1 record")
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toContainText("0 reviewed outcomes")
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toContainText(
  ...
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toContainText(
    "Final-gate follow-through outcome commit history is deterministic review data only",
  )
  await expect(liveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toContainText(
...
async function assertOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterExecutionOutcomeCommitHistory(
  nonCncDemos: Locator,
  page: Page,
) {
  const providerAdapterExecutionOutcomeCommitHistory = nonCncDemos.getByLabel(
  ...
  await expect(providerAdapterExecutionOutcomeCommitHistory).toBeVisible()
  await expect(providerAdapterExecutionOutcomeCommitHistory).toHaveAttribute("data-status", "blocked")
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText(
    "Offer export live adapter final gate follow-through provider-adapter execution outcome commit history",
  )
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText(
    "Final-gate provider-adapter execution outcome commit history blocked",
  )
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText(
    "Latest non-CNC final-gate provider-adapter execution outcome commit is blocked",
  )
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText("1 record")
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText("0 reviewed provider outcomes")
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText("Committed 0, blocked 1")
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText(
    "Resolve final-gate provider-adapter execution outcome commit blockers before retrying provider follow-through wiring.",
  )
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText(
    "Final-gate provider-adapter execution outcome commit history is deterministic review data only",
  )
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText("Commit-ready records: None")
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText(
    "Blocked records: non-cnc-final-gate-provider-adapter-execution-outcome-commit:",
  )
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText("Provider commit executions: None")
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText("Provider boundaries: None")
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText("External IDs: None")
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText(
    "Non-CNC final-gate provider-adapter execution outcome commit history",
  )
  await expect(providerAdapterExecutionOutcomeCommitHistory).toContainText("Status counts: blocked")
  await assertNoHorizontalOverflow(page)
}
...
for (const viewport of operatorViewports) {
  test.describe(`guarded non-CNC process previews on ${viewport.label}`, () => {
    ...
    test("reviews previews and copies the estimator summary", async ({ page }) => {
      ...
      await assertOfferExportLiveAdapterFinalGateFollowThroughHistory(nonCncDemos, page)
      ...
      await assertOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterExecutionOutcomeCommitHistory(
        nonCncDemos,
        page,
      )
