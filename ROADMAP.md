...
- Non-CNC registry quote previews with guarded selector/readiness flows, preview-only process input edits, deterministic promotion plans, local promotion records, ready-only draft payloads, visible reviewable command-package descriptors, and deterministic promotion execution audit records before any active RFQ quote or offer mutation.
