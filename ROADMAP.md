...
- Calendar integration plans for RFQ due holds/reminders and offer follow-up scheduling through provider/fallback adapters, including export fixture coverage, persisted connector link lookups, deterministic reviewed reschedule command planning, local reschedule plan persistence snapshots, operator read-model copy/status, deterministic reschedule execution audit rows, and local execution persistence snapshots before provider execution.
...
- Add richer persisted connector and calendar drill-downs for follow-up rescheduling execution; connector drill-downs now show deterministic cross-RFQ history plus stale/blocked operator recovery actions, and calendar follow-up status rows now preview reviewed reschedule actions with provider-safe command planning, local persistence boundaries, operator read-model copy/status, deterministic execution audit rows, and local execution persistence snapshots.
