...
- CAD-like part preview model with attachment ranking, preview modes, safe browser-native image and PDF drawing previews, adapter-backed STEP/DXF metadata preview states and viewport metadata cards, boundary-aware CAD metadata filename matching, extracted dimensions, CAD metadata adapter boundaries for STEP/DXF/PDF, metadata-only fallback behavior, deterministic attachment review states, and manufacturability flags.
