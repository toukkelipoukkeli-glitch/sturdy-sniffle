...
- Non-CNC registry quote previews with guarded selector/readiness flows, preview-only process input edits, deterministic promotion plans, local promotion records, ready-only draft payloads, visible reviewable command-package descriptors, deterministic promotion execution audit records, and local execution persistence snapshots before any active RFQ quote or offer mutation.
