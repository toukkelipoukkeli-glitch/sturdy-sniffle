...
- Deepen Convex cloud readiness with tenant-safe activity/link filters, queue/workload views that mirror the local workspace runtime, and UI wiring for persisted offer release history/status/follow-up reads; provider readiness and release execution persisted-read hydration now have deterministic bridge paths, and offer follow-up activity reads are normalized behind local/Convex adapters with Offer workspace hydration in flight.
