...
- CAD-like part preview model with attachment ranking, preview modes, safe browser-native image previews, extracted dimensions, CAD metadata adapter boundaries for STEP/DXF/PDF, metadata-only fallback behavior, deterministic attachment review states, and manufacturability flags.
...
- Add richer part-review workflows for CAD thumbnails, preview status, browser-image rendering, dimensional checks, manufacturability flags, and operator overrides while keeping heavy geometry parsing behind adapters.
