...
- Visible workspace UI for quote queue priority, process workload, capacity commitment, operator actions, integration health, provider-run review, CAD metadata review, attachment review state, and manufacturability flags with documented Browser/Playwright fallback QA when Computer Use permissions are blocked.
...
- Add persisted connector sync/link drill-downs, provider run audit history filters, offer reply state filters, and calendar follow-up status controls in the React workspace.
