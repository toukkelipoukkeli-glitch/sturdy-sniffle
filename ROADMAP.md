...
- Offer builder domain model with plain-text offer export, PDF-ready offer document content, export fixtures/snapshots, lifecycle timeline, revision history, customer-facing revision/alternate/terms summary text, release-plan send/follow-up summaries, guarded email draft packages, local email draft package persistence snapshots, draft package history summaries and UI surface, local/mock email draft provider outcomes, provider-backed release command outcome helpers, local provider outcome batch persistence/history summaries and UI surface, provider outcome readiness gates, and follow-up task tracking.
