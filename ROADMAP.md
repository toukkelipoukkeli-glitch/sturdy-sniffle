...
- Add richer persisted connector and calendar drill-downs for follow-up rescheduling execution; connector drill-downs now show deterministic cross-RFQ history plus stale/blocked operator recovery actions, and calendar follow-up status rows now preview reviewed reschedule actions with provider-safe command planning, local persistence boundaries, operator read-model copy/status, deterministic execution audit rows, local execution persistence snapshots, execution read-model copy/status, operator-visible dry-run execution status, and operator-visible execution history summaries.
