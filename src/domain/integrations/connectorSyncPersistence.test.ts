...
describe("connector sync persistence", () => {
  it("stores local payload snapshots without requiring Convex", async () => {
    ...
    expect(adapter.snapshot().payloads[0]?.activities).toHaveLength(3)
    expect(adapter.snapshot().payloads[0]?.links.map((link) => link.provider)).toEqual(["gmail", "calendar", "calendar"])
  })
