...
describe("FactoryBid workspace (component)", () => {
  ...
  it("renders the dense operator workspace with the first RFQ selected and a computed quote", async () => {
    ...
    expect(initialPartPreview).toHaveTextContent("3D CAD preview")
    const adapterPreview = within(initialPartPreview).getByLabelText("FB-204-A.step adapter preview")
    expect(adapterPreview).toHaveTextContent("STEP")
    expect(adapterPreview).toHaveTextContent("Aluminum 6082")
    expect(adapterPreview).toHaveTextContent(/cnc milling/i)
    expect(adapterPreview).toHaveTextContent("heuristic")
    const metadataThumbnail = within(initialPartPreview).getByLabelText("FB-204-A.step metadata thumbnail")
    expect(metadataThumbnail).toHaveTextContent("STEP")
    expect(metadataThumbnail).toHaveTextContent("Aluminum 6082")
    expect(metadataThumbnail).toHaveTextContent(/cnc milling/i)
