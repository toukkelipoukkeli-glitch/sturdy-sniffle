...
describe("FactoryBid workspace (component)", () => {
  ...
  it("renders the dense operator workspace with the first RFQ selected and a computed quote", async () => {
    ...
    await waitFor(() => {
      expect(
        within(processDemos).getByLabelText("Non-CNC promoted quote offer creation outcome commit history"),
      ).toHaveTextContent("Local customer-offer creation outcome commit history: 1 record, 0 outcomes, 0 warnings.")
    })
    const promotedQuoteOfferCreationOutcomeCommitHistory = within(processDemos).getByLabelText(
      "Non-CNC promoted quote offer creation outcome commit history",
    )
    expect(promotedQuoteOfferCreationOutcomeCommitHistory).toHaveAttribute("data-status", "blocked")
    expect(promotedQuoteOfferCreationOutcomeCommitHistory).toHaveTextContent("Offer creation outcome commit history")
    ...
    expect(promotedQuoteOfferCreationOutcomeCommitHistory).toHaveTextContent("review only")
    ...
    await waitFor(() => {
      expect(
        within(processDemos).getByLabelText("Non-CNC promoted quote customer export provider commit history"),
      ).toHaveTextContent("No provider commit history")
    })
    const promotedQuoteOfferExportPackageProviderCommitHistory = within(processDemos).getByLabelText(
      "Non-CNC promoted quote customer export provider commit history",
    )
    expect(promotedQuoteOfferExportPackageProviderCommitHistory).toHaveAttribute("data-status", "empty")
    expect(promotedQuoteOfferExportPackageProviderCommitHistory).toHaveTextContent("Offer export provider commit history")
    expect(promotedQuoteOfferExportPackageProviderCommitHistory).toHaveTextContent(
      NON_CNC_PROMOTED_QUOTE_OFFER_EXPORT_PACKAGE_PROVIDER_COMMIT_PERSISTENCE_VERSION,
    )
    expect(promotedQuoteOfferExportPackageProviderCommitHistory).toHaveTextContent(
      "No non-CNC offer export provider commit runs have been recorded yet.",
    )
    expect(promotedQuoteOfferExportPackageProviderCommitHistory).toHaveTextContent("0 runs")
    expect(promotedQuoteOfferExportPackageProviderCommitHistory).toHaveTextContent("0 artifact outcomes")
    expect(promotedQuoteOfferExportPackageProviderCommitHistory).toHaveTextContent(
    ...
    await waitFor(() => {
      expect(
        within(processDemos).getByLabelText("Non-CNC promoted quote customer export live adapter execution plan"),
      ).toHaveTextContent("decision history 1 record")
    })
    const promotedQuoteOfferExportLiveAdapterExecutionPlan = within(processDemos).getByLabelText(
      "Non-CNC promoted quote customer export live adapter execution plan",
    )
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveAttribute("data-status", "blocked")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent(
      "Offer export live adapter execution plan",
    )
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent(
      NON_CNC_PROMOTED_QUOTE_OFFER_EXPORT_LIVE_ADAPTER_EXECUTION_PLAN_VERSION,
    )
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent(
      "Live-adapter execution plan is blocked for registry-demo by 1 blocker; live customer-offer export writes remain disabled.",
    )
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent("5 commands")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent("0 planned commands")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent("0 withheld commands")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent("5 blocked commands")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent("review only")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent("keep review only")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent("Provider commit evidence withheld")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent("Live adapter target withheld")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent(
    ...
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent("Customer-offer export write")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent("blocked · customer offer")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent("No live adapter idempotency key")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent(
      "No persisted non-CNC offer export provider commit records are available.",
    )
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent(
      "Resolve 1 live-adapter execution blocker.",
    )
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent(
      "Do not run live customer-offer export adapters from this plan.",
    )
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent("Decision history: non-cnc-promoted")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent(
    ...
    await waitFor(() => {
      ...
      ).toHaveTextContent("Live-adapter apply execution history blocked")
    })
    const promotedQuoteOfferExportLiveAdapterApplyExecutionHistory = within(processDemos).getByLabelText(
      "Non-CNC promoted quote customer export live adapter apply execution history",
    )
    expect(promotedQuoteOfferExportLiveAdapterApplyExecutionHistory).toHaveAttribute("data-status", "blocked")
    expect(promotedQuoteOfferExportLiveAdapterApplyExecutionHistory).toHaveTextContent(
      "Offer export live adapter apply execution history",
    )
    expect(promotedQuoteOfferExportLiveAdapterApplyExecutionHistory).toHaveTextContent(
      "Latest non-CNC live-adapter apply execution is blocked",
    )
    expect(promotedQuoteOfferExportLiveAdapterApplyExecutionHistory).toHaveTextContent(
      "live customer-offer, file, release-review, export, and connector writes remain disabled.",
    )
    expect(promotedQuoteOfferExportLiveAdapterApplyExecutionHistory).toHaveTextContent("blocked")
    expect(promotedQuoteOfferExportLiveAdapterApplyExecutionHistory).toHaveTextContent("10 commands")
    expect(promotedQuoteOfferExportLiveAdapterApplyExecutionHistory).toHaveTextContent("blocked 10")
    expect(promotedQuoteOfferExportLiveAdapterApplyExecutionHistory).toHaveTextContent(
      "Apply execution history is deterministic review data only; live customer-offer, file, release-review, export, and connector writes stay disabled.",
    )
    expect(promotedQuoteOfferExportLiveAdapterApplyExecutionHistory).toHaveTextContent(
      "Resolve live-adapter apply execution blockers before recording another execution.",
    )
    expect(promotedQuoteOfferExportLiveAdapterApplyExecutionHistory).toHaveTextContent("Target RFQs: None")
    expect(promotedQuoteOfferExportLiveAdapterApplyExecutionHistory).toHaveTextContent("Committed executions: None")
    expect(promotedQuoteOfferExportLiveAdapterApplyExecutionHistory).toHaveTextContent(
    ...
    const promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionHistory = within(
      processDemos,
    ).getByLabelText(
      "Non-CNC promoted quote customer export live adapter final gate follow-through execution history",
    )
    await waitFor(() => {
      expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionHistory).toHaveTextContent(
        "Final-gate follow-through execution history blocked",
      )
    })
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionHistory).toHaveAttribute(
      "data-status",
      "blocked",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionHistory).toHaveTextContent(
      "Offer export live adapter final gate follow-through execution history",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionHistory).toHaveTextContent(
      "Latest non-CNC live-adapter final-gate follow-through execution is blocked",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionHistory).toHaveTextContent(
      "Execution runs",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionHistory).toHaveTextContent("1 run")
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionHistory).toHaveTextContent(
      "Prepared 0, pending 0, applied 0, failed 0, blocked 5",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionHistory).toHaveTextContent(
      "Resolve final-gate follow-through execution blockers before recording another execution.",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionHistory).toHaveTextContent(
      "Follow-throughs: non-cnc-promoted-quote-offer-export-live-adapter-apply-execution-final-gate-follow-through-",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionHistory).toHaveTextContent(
      "Readiness records: None",
    )
    ...
    await waitFor(() => {
      expect(
        within(processDemos).getByLabelText(
          "Non-CNC promoted quote customer export live adapter final gate follow-through execution outcome commit history",
        ),
      ).toHaveTextContent("Final-gate follow-through outcome commit history blocked")
    })
    const promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory = within(
      processDemos,
    ).getByLabelText(
      "Non-CNC promoted quote customer export live adapter final gate follow-through execution outcome commit history",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toHaveAttribute(
      "data-status",
      "blocked",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toHaveTextContent(
    ...
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toHaveTextContent(
      "Latest non-CNC live-adapter final-gate follow-through outcome commit is blocked after 1 record; live customer-offer, file, release-review, export, connector, and final-gate follow-through writes remain disabled.",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toHaveTextContent(
      "1 record",
    )
    ...
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toHaveTextContent(
      "Committed 0, blocked 1",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toHaveTextContent(
      "Resolve final-gate follow-through outcome commit blockers before retrying release follow-through wiring.",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toHaveTextContent(
      "Final-gate follow-through outcome commit history is deterministic review data only; live customer-offer, file, release-review, export, connector, and final-gate follow-through writes stay disabled.",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toHaveTextContent(
      "Commit-ready records: None",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toHaveTextContent(
      "Blocked records: non-cnc-final-gate-follow-through-outcome-commit:",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitHistory).toHaveTextContent(
      "Follow-throughs: non-cnc-promoted-quote-offer-export-live-adapter-apply-execution-final-gate-follow-through-",
    )
    ...
    await waitFor(() => {
      ...
      ).toHaveTextContent("Final-gate follow-through adapter boundary history blocked")
    })
    const promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory =
      within(processDemos).getByLabelText(
        "Non-CNC promoted quote customer export live adapter final gate follow-through execution outcome commit adapter boundary history",
      )
    expect(
    ...
    expect(
      promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory,
    ).toHaveTextContent("Offer export live adapter final gate follow-through adapter-boundary history")
    expect(
      promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory,
    ).toHaveTextContent(
    ...
    expect(
      ...
      "Latest non-CNC live-adapter final-gate follow-through adapter boundary is blocked after 1 record; live customer-offer, file, release-review, export, connector, and final-gate follow-through writes remain disabled.",
    )
    expect(
      promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory,
    ...
    expect(
      promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory,
    ).toHaveTextContent("6 commands")
    expect(
      promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory,
    ).toHaveTextContent("Planned 0, blocked 6")
    expect(
      promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory,
    ).toHaveTextContent("Resolve final-gate follow-through adapter-boundary blockers before retrying follow-through review.")
    expect(
      promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory,
    ).toHaveTextContent(
      "Final-gate follow-through adapter-boundary history is deterministic review data only; live customer-offer, file, release-review, export, connector, and final-gate follow-through writes stay disabled.",
    )
    expect(
      promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory,
    ).toHaveTextContent("Ready boundaries: None")
    expect(
      promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory,
    ).toHaveTextContent(
    ...
    expect(
      promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory,
    ).toHaveTextContent("Commit records: None")
    expect(
      promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory,
    ).toHaveTextContent("Idempotency keys: None")
    expect(
      promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory,
    ).toHaveTextContent("Non-CNC live adapter final-gate follow-through adapter boundary history")
    expect(
      promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughExecutionOutcomeCommitAdapterBoundaryHistory,
    ...
    await waitFor(() => {
      ...
      ).toHaveTextContent("Final-gate follow-through provider-adapter history blocked")
    })
    const promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterBoundaryHistory =
      within(processDemos).getByLabelText(
        "Non-CNC promoted quote customer export live adapter final gate follow-through provider adapter boundary history",
      )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterBoundaryHistory).toHaveAttribute(
      "data-status",
      "blocked",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterBoundaryHistory).toHaveTextContent(
    ...
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterBoundaryHistory).toHaveTextContent(
      "Latest non-CNC final-gate follow-through provider-adapter boundary is blocked after 1 record; provider command evidence remains withheld and live writes remain disabled.",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterBoundaryHistory).toHaveTextContent(
      "1 record",
    )
    ...
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterBoundaryHistory).toHaveTextContent(
      "Final-gate follow-through provider-adapter boundary history is deterministic review data only; live customer-offer, file, release-review, export, connector, and final-gate follow-through writes stay disabled by default.",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterBoundaryHistory).toHaveTextContent(
      "Ready boundaries: None",
    )
    ...
    await waitFor(() => {
      expect(
        within(processDemos).getByLabelText(
          "Non-CNC promoted quote customer export live adapter final gate follow-through provider adapter execution history",
        ),
      ).toHaveTextContent("Final-gate provider-adapter execution history blocked")
    })
    const promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterExecutionHistory =
      within(processDemos).getByLabelText(
        "Non-CNC promoted quote customer export live adapter final gate follow-through provider adapter execution history",
      )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterExecutionHistory).toHaveAttribute(
      "data-status",
      "blocked",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterExecutionHistory).toHaveTextContent(
    ...
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterExecutionHistory).toHaveTextContent(
      "Latest non-CNC final-gate provider-adapter execution is blocked after 1 run; provider execution evidence remains withheld and live writes remain disabled.",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterExecutionHistory).toHaveTextContent(
      "1 run",
    )
    ...
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterExecutionHistory).toHaveTextContent(
      "Prepared 0, pending 0, applied 0, failed 0, blocked 6",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterExecutionHistory).toHaveTextContent(
      "Resolve final-gate provider-adapter execution blockers before recording provider outcomes.",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterExecutionHistory).toHaveTextContent(
      "Final-gate follow-through provider-adapter execution history is deterministic review data only; live customer-offer, file, release-review, export, connector, and final-gate follow-through writes stay disabled by default.",
    )
    expect(promotedQuoteOfferExportLiveAdapterFinalGateFollowThroughProviderAdapterExecutionHistory).toHaveTextContent(
    ...
    const promotedQuoteOfferExportLiveAdapterExecutionHistory = within(processDemos).getByLabelText(
      "Non-CNC promoted quote customer export live adapter execution history",
    )
    expect(promotedQuoteOfferExportLiveAdapterExecutionHistory).toHaveAttribute("data-status", "blocked")
    expect(promotedQuoteOfferExportLiveAdapterExecutionHistory).toHaveTextContent(
      "Offer export live adapter execution history",
    )
    expect(promotedQuoteOfferExportLiveAdapterExecutionHistory).toHaveTextContent(
    ...
    expect(promotedQuoteOfferExportLiveAdapterExecutionHistory).toHaveTextContent(
      "Live-adapter execution history is deterministic review data only; active RFQ quote, customer offer, file, release-review, export, and connector state stay unchanged.",
    )
    expect(promotedQuoteOfferExportLiveAdapterExecutionHistory).toHaveTextContent("Release executions: None")
    expect(promotedQuoteOfferExportLiveAdapterExecutionHistory).toHaveTextContent("Source executions: None")
    expect(promotedQuoteOfferExportLiveAdapterExecutionHistory).toHaveTextContent(
      "Non-CNC offer export live adapter execution history",
    )
    expect(promotedQuoteOfferExportLiveAdapterExecutionHistory).toHaveTextContent("Status counts: blocked")
    await waitFor(() => {
      expect(
  ...
  it("hydrates provider run audits through the Convex browser bridge", async () => {
    ...
    window.__FACTORYBID_WORKSPACE_CONVEX__ = {
      ...
      runQuery: async (queryRef, args) => {
        ...
        return [
          {
            _id: "provider-run-convex-draft",
            adapterVersion: "provider-adapter.v1.gemini",
            completedAt: Date.parse("2026-06-20T09:00:02.000Z"),
            createdAt: Date.parse("2026-06-20T09:00:03.000Z"),
            inputHash: "convex-input-hash",
            outputSummary: "Persisted Gemini draft ready.",
            provider: "gemini",
            purpose: "draft",
            rfqId: "convex-rfq-204",
            startedAt: Date.parse("2026-06-20T09:00:00.000Z"),
            status: "succeeded",
          },
        ]
      },
    ...
    await waitFor(() => {
      ...
      expect(providerReview).toHaveTextContent("Recent provider reads")
      expect(providerReview).toHaveTextContent("convex read")
      expect(providerReview).toHaveTextContent("Convex 1 · Local 2 · Errors 0")
    ...
    expect(integrationHealth).toHaveTextContent("Recent provider reads")
    expect(integrationHealth).toHaveTextContent("convex read")
  ...
  it("surfaces provider run read fallback health when Convex reads fail", async () => {
    ...
    await waitFor(() => {
      ...
      expect(providerReview).toHaveTextContent("Recent provider reads")
      expect(providerReview).toHaveTextContent("fallback read")
      expect(providerReview).toHaveTextContent("fallback")
      expect(providerReview).toHaveTextContent("Convex 0 · Local 2 · Errors 1")
    ...
    expect(integrationHealth).toHaveTextContent("fallback read")
    expect(integrationHealth).toHaveTextContent("Convex 0 · Local 2 · Errors 1")
  ...
  it("surfaces pending follow-up activity reads when no persisted records exist", async () => {
    ...
    expect(followUpActivity.getAttribute("aria-label")).toBe("Offer follow-up activity reads")
    expect(
      within(followUpActivity).getByLabelText("Offer follow-up activity read source: Local activity"),
    ).toHaveAttribute("data-status", "local")
    expect(followUpActivity).toHaveTextContent("Follow-up activity history is using 0 follow-up activities.")
  ...
  it("keeps local follow-up activity visible while persisted activity reads are pending", async () => {
    ...
    expect(
      within(followUpActivity).getByLabelText("Offer follow-up activity read source: Checking Convex"),
    ).toHaveAttribute("data-status", "pending")
  ...
  it("replays restored manual follow-up actions into local activity readiness after reload", async () => {
    ...
    await user.click(within(readinessHistory).getByRole("button", { name: "Copy history summary" }))
    expect(writeText).toHaveBeenCalledTimes(1)
    expect(writeText.mock.calls[0]?.[0]).toContain("Follow-up readiness history: offer-follow-up-activity-readiness-history.v1")
    expect(writeText.mock.calls[0]?.[0]).toContain("Records: total 2")
    expect(writeText.mock.calls[0]?.[0]).toContain("Current readiness: recorded 1/1 tasks")
    expect(readinessHistory).toHaveTextContent("Readiness history summary copied for diagnostics.")
    const integrationHealth = screen.getByLabelText("Integration health")
    expect(integrationHealth).toHaveTextContent(
    ...
    expect(integrationHealth).toHaveTextContent("Copy the follow-up readiness persisted-read and history diagnostic export.")
    await user.click(within(integrationHealth).getByRole("button", { name: "Copy readiness diagnostics" }))
    expect(writeText).toHaveBeenCalledTimes(2)
    ...
    expect(integrationHealth).toHaveTextContent("Persistence diagnostics copied with readiness history.")
    await user.click(within(readinessHistory).getByRole("button", { name: "Copy read summary" }))
    expect(writeText).toHaveBeenCalledTimes(3)
    expect(writeText.mock.calls[2]?.[0]).toContain("Follow-up readiness persisted read: ready")
  ...
  it("surfaces ready non-CNC promotion outcome drafts for candidate records", async () => {
    ...
    const promotedQuoteOfferCreationOutcomeCommitHistory = within(selectedPreview).getByLabelText(
      "Non-CNC promoted quote offer creation outcome commit history",
    )
    expect(promotedQuoteOfferCreationOutcomeCommitHistory).toHaveAttribute("data-status", "ready")
    expect(promotedQuoteOfferCreationOutcomeCommitHistory).toHaveTextContent("Offer creation outcome commit history")
    expect(promotedQuoteOfferCreationOutcomeCommitHistory).toHaveTextContent(
      NON_CNC_PROMOTED_QUOTE_OFFER_CREATION_OUTCOME_COMMIT_PERSISTENCE_VERSION,
    )
    expect(promotedQuoteOfferCreationOutcomeCommitHistory).toHaveTextContent(
      "Local customer-offer creation outcome commit history: 0 records, 0 outcomes, 0 warnings.",
    )
    expect(promotedQuoteOfferCreationOutcomeCommitHistory).toHaveTextContent("No local record")
    expect(promotedQuoteOfferCreationOutcomeCommitHistory).toHaveTextContent(
    ...
    expect(promotedQuoteOfferExportPackageProviderCommitPlan).toHaveTextContent("0 artifact outcomes")
    expect(promotedQuoteOfferExportPackageProviderCommitPlan).toHaveTextContent("No execution audit emitted")
    const promotedQuoteOfferExportPackageProviderCommitHistory = within(selectedPreview).getByLabelText(
      "Non-CNC promoted quote customer export provider commit history",
    )
    expect(promotedQuoteOfferExportPackageProviderCommitHistory).toHaveAttribute("data-status", "empty")
    expect(promotedQuoteOfferExportPackageProviderCommitHistory).toHaveTextContent("No provider commit history")
    expect(promotedQuoteOfferExportPackageProviderCommitHistory).toHaveTextContent(
      "No non-CNC offer export provider commit runs have been recorded yet.",
    )
    expect(promotedQuoteOfferExportPackageProviderCommitHistory).toHaveTextContent(
    ...
    const promotedQuoteOfferExportLiveAdapterExecutionPlan = within(selectedPreview).getByLabelText(
      "Non-CNC promoted quote customer export live adapter execution plan",
    )
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveAttribute("data-status", "blocked")
    expect(promotedQuoteOfferExportLiveAdapterExecutionPlan).toHaveTextContent(
  ...
  it("records manager release review and unlocks the release plan once the RFQ is ready", async () => {
    ...
    expect(within(integrationHealth).getByLabelText("Email draft package reads recovery actions")).toHaveTextContent(
      "Configure Convex read",
    )
